...
# This script is intentionally serial and uses only GPU 0.
models=(
  ggpl_dynonly_ablation
  ggpl_dynonly_ablation_no_channel
  ggpl_dynonly_ablation_linear_tokenizer
  ggpl_dynonly_ablation_no_graph
  ggpl_dynonly_ablation_linear_tokenizer_no_channel
  ggpl_dynonly_ablation_no_graph_no_channel
  ggpl_dynonly_ablation_linear_tokenizer_no_graph
  ggpl_dynonly_ablation_linear_tokenizer_no_graph_no_channel
)
  ...
  for dataset in "${datasets[@]}"; do
    for seed in "${seeds[@]}"; do
      log="logs/${model}_${dataset}_seed${seed}.log"
      {
        ...
        echo "readout=cls config=configs/default/${model}.yaml"
