...
DEFAULT_DATASETS = ["Ailerons", "diamonds", "medical_charges"]
...
def write_config(base_config, params, output_file):
    ...
    config["model"]["model_name"] = "cgr_tmlp_stage2"
...
def load_prediction(results_dir, model_suffix, dataset):
    prediction_file = results_dir / f"cgr_tmlp_stage2{model_suffix}" / dataset / "prediction.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Small key-hyperparameter search for cgr_tmlp_stage2."
    )
    ...
    parser.add_argument("--base-config", default="configs/default/cgr_tmlp_stage2.yaml")
    parser.add_argument("--work-dir", default="artifacts/cgr_tmlp_stage2_tuning")
    ...
    for params in build_grid(args.mode):
        ...
        config_file = config_dir / f"cgr_tmlp_stage2{tag}.yaml"
