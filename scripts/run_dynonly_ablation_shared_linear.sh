...
models=(
  ...
  ggpl_dynonly_ablation_shared_linear_no_channel
  ggpl_dynonly_ablation_shared_linear_no_graph
  ggpl_dynonly_ablation_shared_linear_no_graph_no_channel
)
  ...
  for dataset in "${datasets[@]}"; do
    for seed in "${seeds[@]}"; do
      log="logs/${model}_${dataset}_seed${seed}.log"
      {
        ...
        echo "tokenizer=shared_linear readout=cls config=configs/default/${model}.yaml"
