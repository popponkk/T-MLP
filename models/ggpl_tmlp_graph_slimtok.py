...
class StaticGuidedDynamicGraphSlimTokBlock(nn.Module):
    """Static graph guides dynamic graph construction; propagation stays dynamic."""
    ...
    def __init__(
        ...
        graph_dynamic_scale_init: float = 1e-2,
        graph_self_loop_init: float = 2.0,
        static_guided_beta: float = 0.1,
        static_temperature: float = 1.0,
    ) -> None:
        super().__init__()
        self.n_tokens = n_tokens
        self.d_token = d_token
        self.graph_dynamic_rank = graph_dynamic_rank
        self.graph_temperature = float(graph_temperature)
        self.static_temperature = float(static_temperature)
        self.static_guided_beta = float(static_guided_beta)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Static graph is used only to guide the construction of the dynamic graph.
        # The final token propagation is still performed by the dynamic graph attention,
        # which avoids directly adding static logits to dynamic logits.
        static_temperature = max(self.static_temperature, 1e-6)
        a_static = torch.softmax(
            self.graph_logits.unsqueeze(0) / static_temperature, dim=-1
        )
        h_static = torch.matmul(a_static, h)
        h_guided = h + self.static_guided_beta * (h_static - h)

        z = self.graph_dynamic_proj(h_guided)
        dynamic_logits = torch.matmul(z, z.transpose(1, 2)) / math.sqrt(
            self.graph_dynamic_rank
        )
        temperature = max(self.graph_temperature, 1e-6)
...
class _GGPLTMLPGraphSlimTok(nn.Module):
    """GGPL-TMLP with feature relation graph token mixing."""
...
class _GGPLTMLPGraphSlimTokSGDyn(nn.Module):
    """GGPL-TMLP with static-guided dynamic graph token mixing."""
    ...
    def __init__(
        ...
        **_: ty.Any,
        ...
        self.layers = nn.ModuleList(
            [
                StaticGuidedDynamicGraphSlimTokBlock(
                    n_tokens=n_tokens,
                    d_token=d_token,
                    channel_hidden=channel_hidden,
                    dropout=dropout or 0.0,
                    ...
                    graph_dynamic_scale_init=graph_dynamic_scale_init,
                    graph_self_loop_init=graph_self_loop_init,
                    static_guided_beta=static_guided_beta,
    ...
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_graph_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGraphSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_graph_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_graph_slimtok_breakpoints"
        )
...
class GGPLTMLPGraphSlimTokSGDyn(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_graph_slimtok_sgdyn keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGraphSlimTokSGDyn(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_graph_slimtok_sgdyn"
        self.device = torch.device(device)
        self.breakpoint_init = self.saved_model_config.get("breakpoint_init", "gbdt")
        self.breakpoint_fallback = self.saved_model_config.get(
            "breakpoint_fallback", "quantile"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_graph_slimtok_sgdyn_breakpoints"
        )
        self.num_breakpoints = int(self.saved_model_config.get("num_breakpoints", 8))
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("graph_dynamic_scale_init", 1e-2)
        model_config.setdefault("graph_self_loop_init", 2.0)
        model_config.setdefault("static_guided_beta", 0.1)
        model_config.setdefault("static_temperature", 1.0)
        return model_config
