...
class LinearHint(nn.Module):
    def __init__(self, topk_max: int, hint_dim: int) -> None:
        super().__init__()
        self.topk_max = int(topk_max)
        hint_dim = max(int(hint_dim), 4)
        self.proj = nn.Sequential(
            nn.Linear(2 * self.topk_max, hint_dim),
            nn.ReLU(),
            nn.Linear(hint_dim, hint_dim),
        )
        self._init()
    ...
    def forward(self, topk_x: Tensor, topk_g: Tensor) -> Tensor:
        topk_w = topk_g / (topk_g.sum(dim=1, keepdim=True) + 1e-6)
        topk_xw = topk_x * topk_w
        if topk_xw.shape[1] < self.topk_max:
            pad = self.topk_max - topk_xw.shape[1]
            topk_xw = F.pad(topk_xw, (0, pad))
            topk_g = F.pad(topk_g, (0, pad))
        return self.proj(torch.cat([topk_xw, topk_g], dim=1))


class _SCGRTMLP(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        spec_output_scale: float = 0.12,
        **backbone_config,
    ) -> None:
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("scgr_tmlp currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("scgr_tmlp requires numerical features")
        if use_hidmix:
            raise NotImplementedError("scgr_tmlp does not implement hidmix")
        ...
        self.lin_hint = LinearHint(self.topk_max, lin_hint_dim)
        self.spec_head = ResidualMLP(
            d_token + top_summary_dim + lin_hint_dim,
            hidden_ratio=spec_hidden_ratio,
            out_dim=1,
            zero_last=False,
        )
        self.conf_gate = ResidualMLP(
            d_token + d_out + 3 + 1 + 1 + d_out + d_out,
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        z_lin_hint = self.lin_hint(topk_x, topk_g)

        spec_input = torch.cat([h_base, z_top, z_lin_hint], dim=1)
        ...
        z_lin_hint_norm = z_lin_hint.norm(dim=1, keepdim=True)
        ...
        # Confidence-gated structured correction; safe residual is always active.
...
class SCGRTMLP(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("scgr_tmlp does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _SCGRTMLP(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "scgr_tmlp"
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("lin_hint_dim", 16)
        model_config.setdefault("gamma_init_bias", -2.5)
        model_config.setdefault("spec_output_scale", 0.12)
        return model_config
    ...
    def fit(
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            for batch in train_loader:
                ...
                z_lin_hint_vals.append(float(extras["z_lin_hint"].detach().norm(dim=1).mean().cpu().item()))
