...
class _CGRTMLP(nn.Module):
    def __init__(
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("cgr_tmlp currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("cgr_tmlp requires numerical features")
        if use_hidmix:
            raise NotImplementedError("cgr_tmlp does not implement hidmix")
...
class CGRTMLP(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("cgr_tmlp does not support sparse gating options yet")
        super().__init__()
