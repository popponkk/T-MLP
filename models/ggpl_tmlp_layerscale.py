...
class _GGPLTMLPLayerScale(nn.Module):
    """Original TMLP backbone with GGPL tokenizer and LayerScale residuals."""
    ...
    def __init__(
        ...
        *,
        ...
        self.layer_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(1) * layer_scale_init) for _ in range(n_layers)]
        )
    ...
    def forward(self, x_num: ty.Optional[Tensor], x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            x = x_residual + self.layer_scales[layer_idx] * x
...
class GGPLTMLPLayerScale(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_layerscale keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPLayerScale(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_layerscale"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_layerscale_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("layer_scale_init", 1e-2)
        return model_config
    ...
    def _extract_gbdt_thresholds(
        ...
        try:
            ...
            print(f"[ggpl_tmlp_layerscale] GBDT threshold extraction failed, falling back: {err}")
        return thresholds
    ...
    def _fit_or_load_breakpoints(
        ...
        if cache_file.exists():
            ...
            print(f"[ggpl_tmlp_layerscale] loaded breakpoints: {cache_file}")
            return
        ...
        print(f"[ggpl_tmlp_layerscale] saved breakpoints: {cache_file}")
    ...
    def fit(
        ...
        if task != "regression":
            raise NotImplementedError("ggpl_tmlp_layerscale currently supports regression only")
