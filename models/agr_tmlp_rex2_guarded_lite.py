...
class _AGRTMLPRex2GuardedLite(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("agr_tmlp_rex2_guarded_lite currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("agr_tmlp_rex2_guarded_lite requires numerical features")
        if num_residual_experts != 2:
            raise NotImplementedError("agr_tmlp_rex2_guarded_lite fixes num_residual_experts=2")
        if use_hidmix:
            raise NotImplementedError("agr_tmlp_rex2_guarded_lite does not implement hidmix")
        ...
        self.safe_residual_head = ResidualMLP(
            d_token,
            hidden_ratio=safe_residual_hidden_ratio,
            out_dim=d_out,
            zero_last=True,
        )
        ...
        self.gamma_head = ResidualMLP(
            d_token + d_out + d_out + 3,
            hidden_ratio=0.25,
            out_dim=1,
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        gamma_input = torch.cat(
            [h_base, y_base, delta_safe, alpha_max, alpha_entropy, expert_gap],
            dim=1,
        )
        gamma = self.gamma_cap * torch.sigmoid(self.gamma_head(gamma_input))

        # Guarded expert refinement: safe residual stays primary, experts only contribute softly.
        y_hat = y_base + beta * (delta_safe + gamma * delta_expert)
...
class AGRTMLPRex2GuardedLite(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError("agr_tmlp_rex2_guarded_lite does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _AGRTMLPRex2GuardedLite(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "agr_tmlp_rex2_guarded_lite"
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("safe_residual_hidden_ratio", 0.5)
        ...
        model_config.setdefault("gamma_init_bias", -3.5)
        model_config.setdefault("gamma_cap", 0.35)
        model_config.setdefault("expert_output_scale_init", 0.1)
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            for batch in train_loader:
                ...
                if self.model.use_corr_reg:
                    corr_term = extras["delta_safe"].pow(2).mean() + extras["delta_expert"].pow(2).mean()
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[rex2_guarded] beta_mean={np.mean(beta_vals):.6g}"
