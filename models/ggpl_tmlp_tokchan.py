...
class ChannelMixing(nn.Module):
    def __init__(self, d_token: int, d_channel: int) -> None:
        super().__init__()
        self.down = nn.Linear(d_token, d_channel)
        self.up = nn.Linear(d_channel, d_token)

    def forward(self, x: Tensor) -> Tensor:
        return self.up(F.gelu(self.down(x)))


class _GGPLTMLPTokChan(nn.Module):
    """GGPL-TMLP with explicit token mixing then channel mixing blocks."""
    ...
    def forward(self, x_num: ty.Optional[Tensor], x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            h2 = layer["channel_mix"](h2)
            if self.residual_dropout:
                h2 = F.dropout(h2, self.residual_dropout, self.training)
            x = x_stage2_residual + self.channel_scales[layer_idx] * h2
...
class GGPLTMLPTokChan(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_tokchan keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPTokChan(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_tokchan"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_tokchan_breakpoints"
        )
    ...
    def _extract_gbdt_thresholds(
        ...
        try:
            ...
            print(f"[ggpl_tmlp_tokchan] GBDT threshold extraction failed, falling back: {err}")
        return thresholds
    ...
    def _fit_or_load_breakpoints(
        ...
        if cache_file.exists():
            ...
            print(f"[ggpl_tmlp_tokchan] loaded breakpoints: {cache_file}")
            return
        ...
        print(f"[ggpl_tmlp_tokchan] saved breakpoints: {cache_file}")
    ...
    def fit(
        ...
        if task != "regression":
            raise NotImplementedError("ggpl_tmlp_tokchan currently supports regression only")
