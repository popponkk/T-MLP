...
class GraphLevelReadout(nn.Module):
    ...
    def forward(self, cls_list: list[Tensor]) -> Tensor:
        ...
        if self.readout_type == "weighted_sum":
            ...
            out = torch.sum(cls_stack * weights.view(1, -1, 1), dim=1)
...
class _GGPLTMLPGraphReadoutSlimTok(nn.Module):
    """GGPL-TMLP with Graph-SlimTok backbone and cross-level CLS readout."""
    ...
    def __init__(
        ...
        **_: ty.Any,
        ...
        if n_readout_levels < 1:
            raise ValueError("graph readout requires at least one level")
...
class GGPLTMLPGraphReadoutSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_graph_readout_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGraphReadoutSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_graph_readout_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            ...
            "artifacts/ggpl_tmlp_graph_readout_slimtok_breakpoints",
        )
