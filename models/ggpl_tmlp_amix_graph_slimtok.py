...
class AMixGraphSlimTokBlock(nn.Module):
    """Graph propagation with a lightweight multiplicative interaction branch."""
    ...
    def __init__(
        ...
        amix_mul_hidden_factor: float = 1.0,
        amix_mul_scale_init: float = 1e-3,
        amix_use_sigmoid_local: bool = True,
    ):
        super().__init__()
        ...
        self.amix_use_sigmoid_local = bool(amix_use_sigmoid_local)
        ...
        mul_hidden = max(1, int(d_token * amix_mul_hidden_factor))
        self.mul_hidden = mul_hidden
        self.mul_neighbor = nn.Linear(d_token, mul_hidden)
        self.mul_local = nn.Linear(d_token, mul_hidden)
        self.mul_out = nn.Linear(mul_hidden, d_token)
        ...
        self.mul_scale = nn.Parameter(torch.ones(1) * amix_mul_scale_init)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        h_add = torch.matmul(a, h)
        h_add = self.graph_out(h_add)

        neighbor = torch.matmul(a, self.mul_neighbor(h))
        local = self.mul_local(h)
        if self.amix_use_sigmoid_local:
            local = torch.sigmoid(local)
        else:
            local = self.activation(local)

        h_mul = neighbor * local
        h_mul = self.mul_out(h_mul)

        h_mix = h_add + self.mul_scale * h_mul
        h_mix = self.dropout(h_mix)
        x = x_res + self.graph_scale * h_mix
...
class _GGPLTMLPAMixGraphSlimTok(nn.Module):
    """GGPL-TMLP with graph propagation plus multiplicative interactions."""
    ...
    def __init__(
        ...
        **_: ty.Any,
        ...
        self.d_token = d_token
        self.graph_dynamic_rank = graph_dynamic_rank
        self.amix_mul_hidden_factor = amix_mul_hidden_factor
        self.channel_hidden = channel_hidden
        self.layers = nn.ModuleList(
            [
                AMixGraphSlimTokBlock(
                    n_tokens=n_tokens,
                    d_token=d_token,
                    ...
                    amix_mul_hidden_factor=amix_mul_hidden_factor,
                    amix_mul_scale_init=amix_mul_scale_init,
                    amix_use_sigmoid_local=amix_use_sigmoid_local,
                )
...
class GGPLTMLPAMixGraphSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_amix_graph_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPAMixGraphSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_amix_graph_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_amix_graph_slimtok_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("amix_mul_hidden_factor", 1.0)
        model_config.setdefault("amix_mul_scale_init", 1e-3)
        model_config.setdefault("amix_use_sigmoid_local", True)
        return model_config
