...
class _GGPLTMLPGFG(nn.Module):
    """GGPL tokenizer + original TMLP backbone with optional GFG/sparse routing."""
    ...
    def _sp_linear(self, lin: nn.Linear, x: Tensor, z=None):
        if z is None:
            x = x @ lin.weight.T
        else:
    ...
    def _sp_residual(self, x: Tensor, x_residual: Tensor, z=None):
        if z is None:
            return x + x_residual
        return z * x + x_residual

    def _sp_forward(
        self, x_num: Tensor, x_cat: ty.Optional[Tensor], sparser: Sparser
    ) -> Tensor:
...
class GGPLTMLPGFG(TabModel):
    ...
    def __init__(
        ...
        self.model = _GGPLTMLPGFG(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        feat_gate = "shared" if feat_gate == "__default__" else feat_gate
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_gfg_breakpoints"
        )
