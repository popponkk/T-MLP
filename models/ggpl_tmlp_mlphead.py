...
class _GGPLTMLPMLPHead(nn.Module):
    """GGPL tokenizer + original TMLP backbone + lightweight MLP prediction head."""
    ...
    def __init__(
        ...
        *,
        ...
        self.head = SmallMLPHead(d_token, d_out, dropout=head_dropout)
...
class GGPLTMLPMLPHead(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_mlphead keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPMLPHead(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_mlphead"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_mlphead_breakpoints"
        )
    ...
    def _extract_gbdt_thresholds(
        ...
        try:
            ...
            print(
                f"[ggpl_tmlp_mlphead] GBDT threshold extraction failed, falling back: {err}"
            )
        return thresholds
    ...
    def _fit_or_load_breakpoints(
        ...
        if cache_file.exists():
            ...
            print(f"[ggpl_tmlp_mlphead] loaded breakpoints: {cache_file}")
            return
        ...
        print(f"[ggpl_tmlp_mlphead] saved breakpoints: {cache_file}")
    ...
    def fit(
        ...
        if task != "regression":
            raise NotImplementedError(
                "ggpl_tmlp_mlphead currently supports regression only"
            )
