...
class ResidualGatedCalibrationStem(nn.Module):
    def __init__(
        self,
        input_dim: int = 4,
        hidden_dim: int = 16,
        ...
        self.delta_out = nn.Linear(last_dim, 1)
        self.lambda_out = nn.Linear(last_dim, 1)
        self.delta_scale = float(delta_scale)
        self._init()
        nn_init.constant_(self.lambda_out.bias, lambda_init_bias)
    ...
    def _init(self) -> None:
        for module in self.net:
            if isinstance(module, nn.Linear):
                nn_init.normal_(module.weight, mean=0.0, std=1e-3)
                nn_init.zeros_(module.bias)
        nn_init.zeros_(self.delta_out.weight)
        nn_init.zeros_(self.delta_out.bias)
        nn_init.normal_(self.lambda_out.weight, mean=0.0, std=1e-3)
        nn_init.zeros_(self.lambda_out.bias)

    def forward(self, x_views: Tensor, x_raw: Tensor) -> Tensor:
        hidden = self.net(x_views)
        delta_x = self.delta_scale * torch.tanh(self.delta_out(hidden).squeeze(-1))
        lambda_x = torch.sigmoid(self.lambda_out(hidden).squeeze(-1))
        x_in = x_raw + lambda_x * delta_x
        return x_in, delta_x, lambda_x, hidden


class _RGCCGRTMLP(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        delta_scale: float = 0.1,
        featurewise_input_gate: bool = True,
        input_gate_hidden_dim: int = 16,
        lambda_init_bias: float = -3.0,
        **backbone_config,
    ) -> None:
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("rgc_cgr_tmlp currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("rgc_cgr_tmlp requires numerical features")
        if use_hidmix:
            raise NotImplementedError("rgc_cgr_tmlp does not implement hidmix")
        ...
        self.use_piecewise_view = bool(use_piecewise_view)
        self.learnable_breakpoints = bool(learnable_breakpoints)
        if not featurewise_input_gate:
            raise NotImplementedError("rgc_cgr_tmlp currently uses feature-wise input residual gates only")
        ...
        self.calibration_stem = ResidualGatedCalibrationStem(
            input_dim=4,
            hidden_dim=max(int(calib_hidden_dim), int(input_gate_hidden_dim)),
    ...
    def _calibrate_input(self, x_num: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        if self.use_piecewise_view:
            ...
            x_piece = piece_embeddings.mean(dim=-1)
        else:
            x_piece = torch.zeros_like(x_num)
        x_views = torch.stack([x_raw, x_robust, x_quantile, x_piece], dim=-1)
        x_in, delta_x, lambda_x, _ = self.calibration_stem(x_views, x_num)
        return x_in, x_robust, x_quantile, x_piece, delta_x, lambda_x
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        x_in, x_robust, x_quantile, x_piece, delta_x, lambda_x = self._calibrate_input(x_num)
...
class RGCCGRTMLP(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("rgc_cgr_tmlp does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _RGCCGRTMLP(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "rgc_cgr_tmlp"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get("breakpoint_cache_dir", "artifacts/rgc_breakpoints")
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("use_piecewise_view", True)
        ...
        model_config.setdefault("featurewise_input_gate", True)
        model_config.setdefault("input_gate_hidden_dim", 16)
    ...
    def _extract_gbdt_thresholds(self, x_np: np.ndarray, y_np: np.ndarray) -> list[list[float]]:
        ...
        try:
            ...
            print(f"[rgc] GBDT threshold extraction failed, falling back: {err}")
        return thresholds
    ...
    def _fit_or_load_breakpoints(self, x_num: ty.Optional[torch.Tensor], ys: ty.Optional[torch.Tensor], save_path: str) -> None:
        ...
        if cache_file.exists():
            ...
            print(f"[rgc] loaded breakpoints: {cache_file}")
            return
        ...
        print(f"[rgc] saved breakpoints: {cache_file}")
    ...
    def fit(
        ...
        if self.model.use_piecewise_view:
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            for batch in train_loader:
                ...
                lambda_vals.append(float(extras["lambda_x"].detach().mean().cpu().item()))
                delta_x_vals.append(float(extras["delta_x"].detach().abs().mean().cpu().item()))
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[rgc_cgr] gamma_mean={np.mean(gamma_vals):.6g}"
                        f" | abs_delta_safe={np.mean(safe_vals):.6g}"
                        f" | abs_delta_spec={np.mean(spec_vals):.6g}"
                        f" | z_top_norm={np.mean(z_top_vals):.6g}"
                        f" | lambda_mean={np.mean(lambda_vals):.6g}"
                        f" | abs_delta_x={np.mean(delta_x_vals):.6g}"
                    ),
                )
