...
class DynamicGraphSlimTokBlock(nn.Module):
    """Dynamic graph token mixing followed by the original channel mixing."""
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # This block removes the global static graph.
        # Dynamic graph attention is constructed for each sample.
        # The final token propagation is fully determined by dynamic graph attention.
        # This formalizes the dynamic_only ablation as a standalone model.
...
class _GGPLTMLPGraphSlimTokDynGraph(nn.Module):
    """GGPL-TMLP with dynamic-only graph token mixing."""
...
class GGPLTMLPGraphSlimTokDynGraph(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_graph_slimtok_dyngraph keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGraphSlimTokDynGraph(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_graph_slimtok_dyngraph"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            ...
            "artifacts/ggpl_tmlp_graph_slimtok_dyngraph_breakpoints",
        )
