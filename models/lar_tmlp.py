...
class _LARTMLP(nn.Module):
    def __init__(
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("lar_tmlp currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("lar_tmlp requires numerical features")
        if use_hidmix:
            raise NotImplementedError("lar_tmlp does not implement hidmix")
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        delta_lar = (a * topk_x).sum(dim=1, keepdim=True) + b
        ...
        # Confidence-gated local affine correction; safe residual is always active.
        y_hat = y_base + delta_safe + gamma * delta_lar
...
class LARTMLP(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("lar_tmlp does not support sparse gating options yet")
        super().__init__()
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("scale_a", 0.25)
        model_config.setdefault("scale_b", 0.10)
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            for batch in train_loader:
                ...
                if self.model.use_corr_reg:
                    corr_term = extras["delta_safe"].pow(2).mean() + extras["delta_lar"].pow(2).mean()
                ...
                lar_vals.append(float(extras["delta_lar"].detach().abs().mean().cpu().item()))
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[lar] gamma_mean={np.mean(gamma_vals):.6g}"
                        f" | abs_delta_safe={np.mean(safe_vals):.6g}"
                        f" | abs_delta_lar={np.mean(lar_vals):.6g}"
