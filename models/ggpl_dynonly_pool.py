"""Mean-pooling factorial ablations for the dynamic-only Graph-SlimTok model.

This module provides the shared implementation for the eight pool-readout
experiments.  The public variants live in one file per registered model so
their configurations and experiment artifacts remain unambiguous.
"""
...
class LinearNumericTokenizer(nn.Module):
    """Plain per-feature numeric tokens with the same [CLS] + numeric order."""

    def __init__(self, d_numerical: int, d_token: int, bias: bool) -> None:
        super().__init__()
        self.d_numerical = int(d_numerical)
        self.d_token = int(d_token)
        self.weight = nn.Parameter(torch.empty(self.d_numerical, self.d_token))
        self.bias = (
            nn.Parameter(torch.empty(self.d_numerical, self.d_token)) if bias else None
        )
        self.cls_token = nn.Parameter(torch.empty(self.d_token))
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor] = None) -> Tensor:
        if x_cat is not None and x_cat.numel() > 0:
            raise NotImplementedError(
                "ggpl_dynonly_pool ablations support numerical features only; "
                "categorical tokens are not included in the numerical mean pool."
            )
        if x_num is None:
            raise ValueError("LinearNumericTokenizer requires numerical features")
...
class DynamicOnlyPoolBlock(nn.Module):
    """Dynamic-only graph and channel branches, each independently removable."""
    ...
    def __init__(
        ...
        *,
        ...
        if self.use_graph:
            # Keep the legacy dynamic_only parameter layout. graph_logits is
            # intentionally unused by forward, exactly as in that baseline.
            ...
            self.graph_dynamic_proj = nn.Linear(
                d_token, self.graph_dynamic_rank, bias=False
            )
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.use_graph:
            ...
            a = torch.softmax(dynamic_logits / temperature, dim=-1)
            h = torch.matmul(a, h)
            h = self.graph_out(h)
            h = self.dropout(h)
...
class _GGPLDynOnlyPoolModel(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        **_: ty.Any,
        ...
        if categories:
            raise NotImplementedError(
                "ggpl_dynonly_pool ablations support pure numerical datasets only; "
                "categorical features would not belong in the numerical mean pool."
            )
        self.d_numerical = int(d_numerical)
        ...
        self.d_token = int(d_token)
        self.channel_hidden = max(1, int(d_token * slimtok_channel_ratio))
        dropout = (
            slimtok_dropout
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor] = None) -> Tensor:
        ...
        for layer in self.layers:
            x = layer(x)
        # Token 0 is CLS. Pool only the original numerical token range.
        x = x[:, 1 : 1 + self.d_numerical].mean(dim=1)
        x = self.normalization(x)
        x = self.activation(x)
        return self.head(x).squeeze(-1)


class _BaseGGPLDynOnlyPool(GGPLTMLP):
    """Wrapper shared by the eight explicitly named pool-readout variants."""
    ...
    def __init__(
        ...
        if categories:
            raise NotImplementedError(
                ...
                "categorical tokens are excluded from its numerical mean pool."
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(dict(model_config))
        self.model = _GGPLDynOnlyPoolModel(
        ...
        if self.use_ggpl_tokenizer:
            ...
            self.num_breakpoints = int(
                self.saved_model_config.get("num_breakpoints", 8)
            )
        else:
        ...
        print(
            ...
            f"readout=numerical_mean parameters={n_parameters}"
        )
    ...
    def _fit_or_load_breakpoints(
        ...
        if self.use_ggpl_tokenizer:
            return super()._fit_or_load_breakpoints(x_num, ys, save_path)
        return None
    ...
    def fit(
        ...
        ys: ty.Optional[Tensor] = None,
        ids: ty.Optional[Tensor] = None,
        y_std: ty.Optional[float] = None,
        eval_set=None,
        patience: int = 0,
        task: str = None,
        training_args: ty.Optional[dict] = None,
        ...
        if task != "regression":
            raise NotImplementedError("ggpl_dynonly_pool supports regression only")
...
class GGPLDynOnlyPool(_BaseGGPLDynOnlyPool):
    model_name = "ggpl_dynonly_pool"
