...
class RefineMLP(nn.Module):
    def __init__(self, d_token: int, d_refine: int) -> None:
        super().__init__()
        self.down = nn.Linear(d_token, d_refine)
        self.up = nn.Linear(d_refine, d_token)

    def forward(self, x: Tensor) -> Tensor:
        return self.up(F.gelu(self.down(x)))


class _GGPLTMLPSGURefine(nn.Module):
    """GGPL-TMLP with a two-stage backbone block: SGU mix then light refine."""
    ...
    def __init__(
        ...
        *,
        ...
        learnable_breakpoints: bool = True,
        d_refine: ty.Optional[int] = None,
        refine_scale_init: float = 1e-2,
        d_out: int,
    ) -> None:
        super().__init__()
        ...
        self.d_ffn = d_hidden
        d_refine = d_refine or max(d_token // 4, 32)
        ...
        for _ in range(n_layers):
            layer = nn.ModuleDict(
                {
                    "stage1_norm": make_normalization(),
                    "linear0": nn.Linear(d_token, d_hidden * 2),
                    "sgu": SGU(n_tokens),
                    "linear1": nn.Linear(d_hidden, d_token),
                    "stage2_norm": make_normalization(),
                    "refine": RefineMLP(d_token, d_refine),
                }
            )
            self.layers.append(layer)
        self.refine_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(1) * refine_scale_init) for _ in range(n_layers)]
        )
    ...
    def forward(self, x_num: ty.Optional[Tensor], x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            h2 = layer["stage2_norm"](x1)
            h2 = layer["refine"](h2)
            if self.residual_dropout:
                h2 = F.dropout(h2, self.residual_dropout, self.training)
            x = x_stage2_residual + self.refine_scales[layer_idx] * h2
...
class GGPLTMLPSGURefine(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_sgurefine keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPSGURefine(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_sgurefine"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_sgurefine_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("d_refine", None)
        model_config.setdefault("refine_scale_init", 1e-2)
        return model_config
    ...
    def _extract_gbdt_thresholds(
        ...
        try:
            ...
            print(f"[ggpl_tmlp_sgurefine] GBDT threshold extraction failed, falling back: {err}")
        return thresholds
    ...
    def _fit_or_load_breakpoints(
        ...
        if cache_file.exists():
            ...
            print(f"[ggpl_tmlp_sgurefine] loaded breakpoints: {cache_file}")
            return
        ...
        print(f"[ggpl_tmlp_sgurefine] saved breakpoints: {cache_file}")
    ...
    def fit(
        ...
        if task != "regression":
            raise NotImplementedError("ggpl_tmlp_sgurefine currently supports regression only")
