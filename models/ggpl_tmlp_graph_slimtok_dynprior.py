...
class DynamicDominantStaticPriorGraphSlimTokBlock(nn.Module):
    """Dynamic-dominant graph attention with static prior reweighting."""
    ...
    def __init__(
        ...
        static_prior_beta: float = 0.05,
        static_temperature: float = 1.0,
        static_prior_eps: float = 1e-6,
    ) -> None:
        super().__init__()
        ...
        self.static_prior_beta = float(static_prior_beta)
        self.static_prior_eps = float(static_prior_eps)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Dynamic attention is the main graph.
        # Static graph is converted into a weak prior distribution.
        # The final attention is obtained by prior reweighting rather than logits addition.
        # This avoids directly adding static logits to dynamic logits.
        dynamic_temperature = max(self.graph_temperature, 1e-6)
        static_temperature = max(self.static_temperature, 1e-6)
        a_dynamic = torch.softmax(dynamic_logits / dynamic_temperature, dim=-1)
        a_static = torch.softmax(
            self.graph_logits.unsqueeze(0) / static_temperature, dim=-1
        )
        a = a_dynamic * torch.pow(
            a_static + self.static_prior_eps, self.static_prior_beta
        )
        a = a / (a.sum(dim=-1, keepdim=True) + self.static_prior_eps)
...
class _GGPLTMLPGraphSlimTokDynPrior(nn.Module):
    """GGPL-TMLP with dynamic-dominant static-prior graph token mixing."""
    ...
    def __init__(
        ...
        **_: ty.Any,
        ...
        self.layers = nn.ModuleList(
            [
                DynamicDominantStaticPriorGraphSlimTokBlock(
                    n_tokens=n_tokens,
                    d_token=d_token,
                    ...
                    static_prior_beta=static_prior_beta,
                    static_temperature=static_temperature,
                    static_prior_eps=static_prior_eps,
                )
...
class GGPLTMLPGraphSlimTokDynPrior(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_graph_slimtok_dynprior keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGraphSlimTokDynPrior(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_graph_slimtok_dynprior"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            ...
            "artifacts/ggpl_tmlp_graph_slimtok_dynprior_breakpoints",
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("static_prior_beta", 0.05)
        model_config.setdefault("static_temperature", 1.0)
        model_config.setdefault("static_prior_eps", 1e-6)
        return model_config
