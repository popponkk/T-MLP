...
class SlimTokBlock(nn.Module):
    """Original low-rank token mixing followed by channel mixing."""
...
class TabMEnsembleHead(nn.Module):
    def __init__(
        ...
        d_out: int,
        n_members: int = 8,
        member_init_std: float = 0.02,
        member_dropout: float = 0.0,
    ):
        super().__init__()
        self.n_members = n_members
        self.d_out = d_out
        self.member_scale = nn.Parameter(torch.empty(n_members, d_token))
        self.member_bias = nn.Parameter(torch.empty(n_members, d_token))
        self.shared_head = nn.Linear(d_token, d_out)
        self.member_out_bias = nn.Parameter(torch.zeros(n_members, d_out))
        self.dropout = nn.Dropout(member_dropout)

        nn.init.normal_(self.member_scale, mean=1.0, std=member_init_std)
        nn.init.normal_(self.member_bias, mean=0.0, std=member_init_std)
        nn.init.zeros_(self.member_out_bias)

    def forward(self, h: Tensor) -> Tensor:
        h_members = (
            h.unsqueeze(1) * self.member_scale.unsqueeze(0)
            + self.member_bias.unsqueeze(0)
        )
        h_members = self.dropout(h_members)
        preds = self.shared_head(h_members) + self.member_out_bias.unsqueeze(0)
        y = preds.mean(dim=1)
        return y.squeeze(-1)


class _GGPLTMLPTabMSlimTok(nn.Module):
    """GGPL-TMLP with original SlimTok backbone and TabM-style ensemble head."""
    ...
    def __init__(
        ...
        *,
        ...
        tabm_n_members: int = 8,
        tabm_member_init_std: float = 0.02,
        tabm_member_dropout: float = 0.0,
        d_out: int,
    ) -> None:
        super().__init__()
        ...
        self.tabm_n_members = tabm_n_members
        ...
        self.head = TabMEnsembleHead(
            d_token=d_token,
            d_out=d_out,
            n_members=tabm_n_members,
            member_init_std=tabm_member_init_std,
            member_dropout=tabm_member_dropout,
        )
...
class GGPLTMLPTabMSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_tabm_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPTabMSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_tabm_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_tabm_slimtok_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("tabm_n_members", 8)
        model_config.setdefault("tabm_member_init_std", 0.02)
        model_config.setdefault("tabm_member_dropout", 0.0)
        return model_config
