...
class _AGRTMLPRex2Lite(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("agr_tmlp_rex2_lite currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("agr_tmlp_rex2_lite requires numerical features")
        if num_residual_experts != 2:
            raise NotImplementedError("agr_tmlp_rex2_lite fixes num_residual_experts=2")
        if use_hidmix:
            raise NotImplementedError("agr_tmlp_rex2_lite does not implement hidmix")
        ...
        # Reuse AGR-style stabilized gated input as the backbone entry point.
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        if self.use_router:
            alpha = torch.softmax(self.router(router_input) / max(self.router_temperature, 1e-6), dim=-1)
        else:
            alpha = torch.full(
                (h_base.shape[0], 2),
                0.5,
        ...
        delta_expert = alpha[:, 0:1] * e1 + alpha[:, 1:2] * e2

        beta = torch.sigmoid(self.beta_head(router_input))
        gamma_input = torch.cat([h_base, y_base, delta_safe], dim=1)
        gamma = torch.sigmoid(self.gamma_head(gamma_input))

        # Final low-disturbance correction: safe residual is primary, experts are secondary.
...
class AGRTMLPRex2Lite(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError("agr_tmlp_rex2_lite does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _AGRTMLPRex2Lite(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "agr_tmlp_rex2_lite"
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("router_temperature", 1.0)
        return model_config
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[rex2] beta_mean={np.mean(beta_vals):.6g}"
