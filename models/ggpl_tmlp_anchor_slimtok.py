...
class AnchorSlimTokBlock(nn.Module):
    """Anchor-based low-rank token mixing followed by channel mixing."""
    ...
    def __init__(
        ...
        self.anchor_in_logits = nn.Parameter(torch.empty(n_tokens, token_rank))
        self.anchor_out_logits = nn.Parameter(torch.empty(token_rank, n_tokens))
        nn.init.normal_(self.anchor_in_logits, mean=0.0, std=anchor_init_std)
        nn.init.normal_(self.anchor_out_logits, mean=0.0, std=anchor_init_std)
    ...
    def _anchor_weights(self) -> tuple[Tensor, Tensor]:
        temperature = max(self.anchor_temperature, 1e-6)
        a = torch.softmax(self.anchor_in_logits / temperature, dim=0)
        b = torch.softmax(self.anchor_out_logits / temperature, dim=-1)
        return a, b
...
class _GGPLTMLPAnchorSlimTok(nn.Module):
    """GGPL-TMLP with anchor-based low-rank token mixing."""
...
class GGPLTMLPAnchorSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_anchor_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPAnchorSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_anchor_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_anchor_slimtok_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("anchor_init_std", 0.02)
        model_config.setdefault("anchor_temperature", 1.0)
        return model_config
