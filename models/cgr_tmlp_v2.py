...
class _CGRTMLPV2(nn.Module):
    def __init__(
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("cgr_tmlp_v2 currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("cgr_tmlp_v2 requires numerical features")
        if use_hidmix:
            raise NotImplementedError("cgr_tmlp_v2 does not implement hidmix")
        ...
        self.rho_raw = nn.Parameter(torch.log(torch.expm1(torch.tensor(float(rho_init)))))
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        y_hat = y_base + delta_safe + rho * gamma * delta_spec
...
class CGRTMLPV2(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("cgr_tmlp_v2 does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _CGRTMLPV2(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "cgr_tmlp_v2"
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("rho_init", 0.5)
        return model_config
    ...
    def fit(
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            for batch in train_loader:
                ...
                rho_vals.append(float(extras["rho"].detach().cpu().item()))
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[cgr_v2] gamma_mean={np.mean(gamma_vals):.6g}"
                        f" | rho={np.mean(rho_vals):.6g}"
