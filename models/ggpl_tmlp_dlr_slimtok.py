...
class DLRSlimTokBlock(nn.Module):
    """Diagonal + low-rank token mixing followed by channel mixing."""
    ...
    def __init__(
        ...
        self.token_diag = nn.Parameter(torch.ones(n_tokens) * dlr_diag_init)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        low = self.token_down(h)
        low = self.activation(low)
        low = self.dropout(low)
        low = self.token_up(low)

        diag = h * self.token_diag.view(1, 1, -1)
        h = low + diag
        h = h.transpose(1, 2)
...
class _GGPLTMLPDLRSlimTok(nn.Module):
    """GGPL-TMLP with diagonal + low-rank slim token mixer backbone."""
...
class GGPLTMLPDLRSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_dlr_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPDLRSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_dlr_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_dlr_slimtok_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("dlr_diag_init", 1.0)
        return model_config
