...
class _NRCGRTMLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        scale_hidden_ratio: float = 0.25,
        gamma_init_bias: float = -2.6,
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("nr_cgr_tmlp currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("nr_cgr_tmlp requires numerical features")
        if use_hidmix:
            raise NotImplementedError("nr_cgr_tmlp does not implement hidmix")
        ...
        self.scale_head = ResidualMLP(
            d_token + d_out + 3,
            hidden_ratio=scale_hidden_ratio,
            out_dim=1,
            last_bias_init=0.0,
            zero_last=False,
        )
        self.conf_gate = ResidualMLP(
            d_token + d_out + 2 + d_out + d_out + d_out,
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        raw_delta_spec_norm = self.spec_head(torch.cat([h_base, z_top], dim=1))
        delta_spec_norm = self.spec_output_scale * torch.tanh(raw_delta_spec_norm)
        scale_input = torch.cat([h_base, y_base, gate_mean, gate_std, z_top_norm], dim=1)
        scale_res = F.softplus(self.scale_head(scale_input)) + self.scale_eps
        spec_residual = scale_res * delta_spec_norm
        ...
        # The specialized head predicts normalized residual; scale_head restores its magnitude.
        y_hat = y_base + delta_safe + gamma * spec_residual
        ...
        return y_hat.squeeze(-1), {
            ...
            "delta_spec_norm": delta_spec_norm,
            "scale_res": scale_res,
...
class NRCGRTMLP(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("nr_cgr_tmlp does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _NRCGRTMLP(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "nr_cgr_tmlp"
        self.device = torch.device(device)
        self.target_transform = self.saved_model_config.get("target_transform", "none")
        self.feature_norm = self.saved_model_config.get("feature_norm", "none")
        self._x_center = None
        self._x_scale = None
        self._y_shift = None
        self._y_scale = None
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.pop("target_transform", None)
        model_config.pop("feature_norm", None)
        ...
        model_config.setdefault("scale_hidden_ratio", 0.25)
        model_config.setdefault("gamma_init_bias", -2.6)
        model_config.setdefault("spec_output_scale", 1.0)
        model_config.setdefault("scale_eps", 1e-4)
        return model_config

    def _fit_feature_transform(self, x_num: ty.Optional[torch.Tensor]) -> None:
        if x_num is None or self.feature_norm == "none":
            self._x_center, self._x_scale = None, None
            return
        if self.feature_norm == "standard":
            center = x_num.mean(dim=0, keepdim=True)
            scale = x_num.std(dim=0, unbiased=False, keepdim=True)
        ...
        self._x_center = center.to(self.device)
        self._x_scale = torch.clamp(scale.to(self.device), min=1e-6)

    def _transform_x(self, x_num: ty.Optional[torch.Tensor]) -> ty.Optional[torch.Tensor]:
        if x_num is None or self._x_center is None or self._x_scale is None:
            return x_num
        return (x_num - self._x_center.to(x_num.device)) / self._x_scale.to(x_num.device)

    def _fit_target_transform(self, y: torch.Tensor) -> None:
        if self.target_transform in ["none", "log1p"]:
            self._y_shift, self._y_scale = None, None
            return
        if self.target_transform == "standardize":
            self._y_shift = y.mean().to(self.device)
            self._y_scale = torch.clamp(y.std(unbiased=False), min=1e-6).to(self.device)
            return
        if self.target_transform == "yeo_johnson":
            raise NotImplementedError("target_transform='yeo_johnson' is reserved but not implemented")
    ...
    def _transform_y(self, y: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_transform == "log1p":
            return torch.log1p(torch.clamp(y, min=-0.999999))
        if self.target_transform == "standardize":
            return (y - self._y_shift.to(y.device)) / self._y_scale.to(y.device)
        raise ValueError(f"Unsupported target_transform: {self.target_transform}")

    def _inverse_y(self, y: torch.Tensor) -> torch.Tensor:
        if self.target_transform == "none":
            return y
        if self.target_transform == "log1p":
            return torch.expm1(y)
        if self.target_transform == "standardize":
            return y * self._y_scale.to(y.device) + self._y_shift.to(y.device)
        raise ValueError(f"Unsupported target_transform: {self.target_transform}")
    ...
    def fit(
        ...
        X_num_raw = X_num
        eval_set_raw = eval_set
        self._fit_feature_transform(X_num_raw)
        X_num_train = self._transform_x(X_num_raw) if X_num_raw is not None else None
        eval_set_train_x = None
        if eval_set_raw is not None:
            eval_set_train_x = tuple(
                (
                    self._transform_x(split[0]),
                    split[1],
                    split[2],
                    split[3],
                )
                for split in eval_set_raw
            )
        self._fit_target_transform(ys)
        ys_train = self._transform_y(ys)
        eval_set_train = None
        if eval_set_train_x is not None:
            eval_set_train = tuple(
                (
                    split[0],
                    split[1],
                    self._transform_y(split[2]),
                    split[3],
                )
                for split in eval_set_train_x
            )
        ...
        if eval_set_raw is not None:
            dev_loader = TabModel.prepare_tensor_loader(
                X_num=eval_set_train[0][0], X_cat=eval_set_train[0][1], ys=eval_set_raw[0][2], ids=eval_set_train[0][3],
                batch_size=training_args["batch_size"],
            )
            test_loader = None
            if len(eval_set_raw) == 2:
                test_loader = TabModel.prepare_tensor_loader(
                    X_num=eval_set_train[1][0], X_cat=eval_set_train[1][1], ys=eval_set_raw[1][2], ids=eval_set_train[1][3],
                    batch_size=training_args["batch_size"],
                )
        else:
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            for batch in train_loader:
                ...
                if self.model.use_corr_reg:
                    corr_term = extras["delta_safe"].pow(2).mean() + extras["spec_residual"].pow(2).mean()
                ...
                spec_vals.append(float(extras["spec_residual"].detach().abs().mean().cpu().item()))
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                if eval_set_raw is not None:
                    val_metrics = self.predict(
                        X_num=eval_set_raw[0][0],
                        X_cat=eval_set_raw[0][1],
                        ys=eval_set_raw[0][2],
                        y_std=y_std,
                        task=task,
                        return_metric=True,
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[nr_cgr] gamma_mean={np.mean(gamma_vals):.6g}"
                        f" | abs_delta_safe={np.mean(safe_vals):.6g}"
                        f" | abs_spec_residual={np.mean(spec_vals):.6g}"
    ...
    def predict(
        ...
        def inference_step(model, x_num, x_cat):
            ...
            predictions = self._inverse_y(predictions)
            used = time.time() - start
            return predictions, used

        if dev_loader is None and X_num is not None:
            X_num = self._transform_x(X_num)
