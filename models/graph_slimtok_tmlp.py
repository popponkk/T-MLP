...
class GraphSlimTokBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        logits = (
            self.graph_logits.unsqueeze(0) + self.graph_dynamic_scale * dynamic_logits
        )
...
class StaticPriorGraphSlimTokBlock(nn.Module):
    """Dynamic graph dominates, static graph acts as a weak tanh prior."""
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        static_prior = torch.tanh(self.graph_logits).unsqueeze(0)
        logits = dynamic_logits + self.static_prior_beta * static_prior
        temperature = max(self.graph_temperature, 1e-6)
        a = torch.softmax(logits / temperature, dim=-1)
...
class _GraphSlimTokTMLP(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
...
class _GraphSlimTokTMLPStaticPrior(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
...
class GraphSlimTokTMLP(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "graph_slimtok_tmlp keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GraphSlimTokTMLP(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "graph_slimtok_tmlp"
...
class GraphSlimTokTMLPStaticPrior(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "graph_slimtok_tmlp_static_prior keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GraphSlimTokTMLPStaticPrior(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "graph_slimtok_tmlp_static_prior"
        self.device = torch.device(device)
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("graph_dynamic_scale_init", 1e-2)
        model_config.setdefault("graph_self_loop_init", 2.0)
        model_config.setdefault("static_prior_beta", 0.1)
        return model_config
