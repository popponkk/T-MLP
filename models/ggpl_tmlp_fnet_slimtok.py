...
class FNetSlimTokBlock(nn.Module):
    """Fourier token mixing followed by channel mixing."""
    ...
    def __init__(
        ...
        activation: str = "gelu",
        fnet_mix_dim: str = "token",
        fnet_use_2d_fft: bool = False,
        fnet_use_out_proj: bool = True,
        fnet_norm: str = "ortho",
    ) -> None:
        super().__init__()
        ...
        self.fnet_mix_dim = fnet_mix_dim
        self.fnet_use_2d_fft = bool(fnet_use_2d_fft)
        self.fnet_use_out_proj = bool(fnet_use_out_proj)
        self.fnet_norm = fnet_norm
        self.n_tokens = int(n_tokens)
        self.d_token = int(d_token)

        if self.fnet_mix_dim != "token":
            raise ValueError(
...
class _GGPLTMLPFNetSlimTok(nn.Module):
    """GGPL-TMLP with Fourier token mixing backbone."""
    ...
    def __init__(
        ...
        **_: ty.Any,
        ...
        self.fnet_mix_dim = fnet_mix_dim
        self.fnet_use_2d_fft = fnet_use_2d_fft
        self.fnet_use_out_proj = fnet_use_out_proj
        ...
        self.layers = nn.ModuleList(
            [
                FNetSlimTokBlock(
                    ...
                    fnet_mix_dim=fnet_mix_dim,
                    fnet_use_2d_fft=fnet_use_2d_fft,
                    fnet_use_out_proj=fnet_use_out_proj,
                    fnet_norm=fnet_norm,
                )
...
class GGPLTMLPFNetSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_fnet_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPFNetSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_fnet_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_fnet_slimtok_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("fnet_mix_dim", "token")
        model_config.setdefault("fnet_use_2d_fft", False)
        model_config.setdefault("fnet_use_out_proj", True)
        model_config.setdefault("fnet_norm", "ortho")
        return model_config
