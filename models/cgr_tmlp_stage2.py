...
class _CGRTMLPStage2(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        spec_output_scale: float = 0.10,
        **backbone_config,
    ) -> None:
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("cgr_tmlp_stage2 currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("cgr_tmlp_stage2 requires numerical features")
        if use_hidmix:
            raise NotImplementedError("cgr_tmlp_stage2 does not implement hidmix")
        ...
        self.disable_specialized = False
...
class CGRTMLPStage2(TabModel):
    def __init__(
        ...
        if feat_gate or pruning:
            raise NotImplementedError("cgr_tmlp_stage2 does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _CGRTMLPStage2(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "cgr_tmlp_stage2"
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("spec_output_scale", 0.10)
        return model_config
    ...
    def _set_stage_trainable(self, stage: str) -> None:
        stage1 = stage == "stage1"
        self.model.disable_specialized = stage1
        self._set_requires_grad(self.model.input_gate, True)
        self._set_requires_grad(self.model.backbone, True)
        self._set_requires_grad(self.model.base_head, True)
        self._set_requires_grad(self.model.safe_residual_head, True)
        self._set_requires_grad(self.model.top_summary, not stage1)
        self._set_requires_grad(self.model.spec_head, not stage1)
        self._set_requires_grad(self.model.conf_gate, not stage1)

    def _make_stage_optimizer(self, training_args: dict, stage: str):
        base_lr = training_args.get("base_lr", training_args.get("lr", 1e-4))
        spec_lr = 0.0 if stage == "stage1" else training_args.get("spec_lr", base_lr)
        ...
        spec_modules = [
            ...
            self.model.spec_head,
            self.model.conf_gate,
        ]
        param_groups = [
            {
            ...
            {
                "params": [p for module in spec_modules for p in module.parameters()],
                "lr": spec_lr,
                "name": "spec",
            },
        ]
        ...
        if optimizer_cls is torch.optim.SGD:
            return optimizer_cls(param_groups, weight_decay=weight_decay, momentum=0.9), None
        return optimizer_cls(param_groups, weight_decay=weight_decay), None
    ...
    def fit(
        ...
        training_args.setdefault("max_epochs", 200)
        ...
        training_args.setdefault("train_mode", "two_stage")
        ...
        optimizer, scheduler = self._make_stage_optimizer(training_args, current_stage)
        self.append_log(
            meta_args["save_path"],
            f"[training] mode={train_mode} | stage={current_stage}"
            f" | lrs={self._format_group_lrs(optimizer)}"
            f" | patience={training_args['patience']}"
            f" | main_loss={training_args['main_loss_type']}",
        )
        ...
        for epoch_idx in range(training_args["max_epochs"]):
            ...
            if desired_stage != current_stage:
                ...
                self.append_log(
                    meta_args["save_path"],
                    f"[stage-switch] epoch={epoch_idx + 1} | stage={current_stage}"
                    f" | lrs={self._format_group_lrs(optimizer)}",
                )
            self.model.train()
            tot_loss = 0.0
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[cgr_stage2] stage={current_stage}"
                        f" | lrs={self._format_group_lrs(optimizer)}"
                ...
                if is_early_stop:
                    ...
                    best_metric = self.history["val"]["best_metric"]
                    self.append_log(
                        meta_args["save_path"],
                        f"[early-stop] best_epoch={self.history['val']['best_epoch']}"
                        f" | stop_epoch={tot_step // steps_per_epoch}"
                        f" | best_val_metric={best_metric}",
                    )
                    return
        self.save(meta_args["save_path"])
        self.load_best_dnn(meta_args["save_path"])
        ...
        self.append_log(
            meta_args["save_path"],
            f"[training-end] best_epoch={self.history['val']['best_epoch']}"
            f" | stop_epoch={tot_step // steps_per_epoch}"
            f" | best_val_metric={best_metric}",
        )
