...
class _AGRTMLPSwitchLite(nn.Module):
    def __init__(
        self,
        *,
        d_numerical: int,
        ...
        **backbone_config,
        ...
        super().__init__()
        if d_out != 1:
            raise NotImplementedError("agr_tmlp_switch_lite currently supports regression only")
        if d_numerical <= 0:
            raise NotImplementedError("agr_tmlp_switch_lite requires numerical features")
        if num_residual_experts != 2:
            raise NotImplementedError("agr_tmlp_switch_lite fixes num_residual_experts=2")
        if use_hidmix:
            raise NotImplementedError("agr_tmlp_switch_lite does not implement hidmix")
    ...
    def forward(self, x_num, x_cat, return_extras: bool = False):
        ...
        mode_probs = torch.softmax(self.mode_gate(mode_input), dim=-1)
...
class AGRTMLPSwitchLite(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError("agr_tmlp_switch_lite does not support sparse gating options yet")
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _AGRTMLPSwitchLite(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "agr_tmlp_switch_lite"
        ...
        for _ in range(training_args["max_epochs"]):
            ...
            if training_args["save_frequency"] == "epoch":
                ...
                self.append_log(
                    meta_args["save_path"],
                    (
                        f"[switch_lite] beta_mean={np.mean(beta_vals):.6g}"
