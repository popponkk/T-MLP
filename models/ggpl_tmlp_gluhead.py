...
from .ggpl_tmlp import GGPLTokenizer


class GLUHead(nn.Module):
    def __init__(self, d_in: int, d_out: int, dropout: float = 0.1) -> None:
        super().__init__()
        d_hidden = max(d_in // 2, 32)
        self.norm = nn.LayerNorm(d_in)
        self.linear0 = nn.Linear(d_in, d_hidden * 2)
        self.dropout = nn.Dropout(dropout)
        self.linear1 = nn.Linear(d_hidden, d_out)
        self._init_weights()

    def _init_weights(self) -> None:
        nn_init.normal_(self.linear0.weight, mean=0.0, std=1e-3)
        nn_init.zeros_(self.linear0.bias)
        nn_init.normal_(self.linear1.weight, mean=0.0, std=1e-3)
...
class _GGPLTMLPGLUHead(nn.Module):
    """GGPL tokenizer + original TMLP backbone + lightweight GLU prediction head."""
    ...
    def __init__(
        ...
        *,
        ...
        # [CLS] readout stays the same; only the final prediction head changes.
        self.readout_norm = nn.LayerNorm(d_token)
        self.readout_act = F.gelu
        self.head = GLUHead(d_token, d_out, dropout=head_dropout)
...
class GGPLTMLPGLUHead(TabModel):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_gluhead keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        super().__init__()
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGLUHead(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_gluhead"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_gluhead_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("head_dropout", 0.1)
        return model_config
    ...
    def _extract_gbdt_thresholds(
        ...
        try:
            ...
            print(
                f"[ggpl_tmlp_gluhead] GBDT threshold extraction failed, falling back: {err}"
            )
        return thresholds
    ...
    def _fit_or_load_breakpoints(
        ...
        if cache_file.exists():
            ...
            print(f"[ggpl_tmlp_gluhead] loaded breakpoints: {cache_file}")
            return
        ...
        print(f"[ggpl_tmlp_gluhead] saved breakpoints: {cache_file}")
    ...
    def fit(
        ...
        if task != "regression":
            raise NotImplementedError(
                "ggpl_tmlp_gluhead currently supports regression only"
            )
