...
class SwapGraphSlimTokBlock(nn.Module):
    """MLP token mixing followed by graph channel mixing."""
    ...
    def __init__(
        ...
        self.channel_graph_scale = nn.Parameter(torch.ones(1) * layerscale_init)
        ...
        with torch.no_grad():
            ...
            self.channel_graph_logits.add_(
                torch.eye(d_token) * graph_self_loop_init
            )
...
class _GGPLTMLPSwapGraphSlimTok(nn.Module):
    """GGPL-TMLP with MLP token mixing and graph channel mixing."""
    ...
    def __init__(
        ...
        *,
        ...
        slimtok_rank_ratio: float = 0.5,
        slimtok_min_rank: int = 16,
        ...
        **_: ty.Any,
        ...
        if slimtok_rank is not None:
            token_rank = int(slimtok_rank)
        else:
            token_rank = max(int(slimtok_min_rank), int(n_tokens * slimtok_rank_ratio))
...
class GGPLTMLPSwapGraphSlimTok(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_swap_graph_slimtok keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPSwapGraphSlimTok(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_swap_graph_slimtok"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            ...
            "artifacts/ggpl_tmlp_swap_graph_slimtok_breakpoints",
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("slimtok_rank_ratio", 0.5)
        model_config.setdefault("slimtok_min_rank", 16)
