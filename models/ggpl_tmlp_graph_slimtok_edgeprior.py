...
class EdgePriorGraphSlimTokBlock(nn.Module):
    """Static graph provides candidate edges, dynamic graph assigns weights."""
    ...
    def __init__(
        ...
        edgeprior_topk: ty.Optional[int] = None,
    ) -> None:
        super().__init__()
        ...
        self.edgeprior_topk = (
            max(1, min(int(edgeprior_topk), n_tokens))
            if edgeprior_topk is not None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        dynamic_mask = _build_topk_mask(dynamic_logits, self.edgeprior_topk)
        static_mask = _build_topk_mask(static_logits, self.edgeprior_topk)
        ...
        cls_mask = torch.zeros_like(dynamic_mask)
        cls_mask[:, 0, :] = True
        cls_mask[:, :, 0] = True

        mask = dynamic_mask | static_mask | self_mask | cls_mask

        # Static graph is used only to provide candidate edges.
        # Dynamic graph still determines the final attention weights.
        # The final attention is computed from masked dynamic logits.
...
class _GGPLTMLPGraphSlimTokEdgePrior(nn.Module):
    """GGPL-TMLP with static-candidate dynamic graph token mixing."""
...
class GGPLTMLPGraphSlimTokEdgePrior(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_graph_slimtok_edgeprior keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPGraphSlimTokEdgePrior(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_graph_slimtok_edgeprior"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            ...
            "artifacts/ggpl_tmlp_graph_slimtok_edgeprior_breakpoints",
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("edgeprior_topk", None)
        return model_config
