...
def _resolve_activation(name: str):
    ...
    if name == "silu":
        return F.silu
    raise ValueError(f"Unsupported crossmix activation: {name}")


class CrossMixBlock(nn.Module):
    """Single backbone block with explicit multiplicative cross interaction."""
    ...
    def __init__(
        ...
        super().__init__()
        self.cross_norm = nn.LayerNorm(d_token)
        self.cross_a = nn.Linear(d_token, cross_hidden)
        self.cross_b = nn.Linear(d_token, cross_hidden)
        self.cross_out = nn.Linear(cross_hidden, d_token)
        ...
        self.cross_scale = nn.Parameter(torch.ones(1) * layerscale_init)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        c = a * b
        c = self.dropout(c)
        delta = self.cross_out(c)
        x = x_res + self.cross_scale * delta

        if self.use_channel_mlp:
            x_res = x
            h2 = self.channel_norm(x)
            h2 = self.channel_down(h2)
            h2 = self.activation(h2)
            h2 = self.dropout(h2)
            h2 = self.channel_up(h2)
            x = x_res + self.channel_scale * h2
        return x


class _GGPLTMLPCrossMix(nn.Module):
    """GGPL-TMLP with CrossMix backbone blocks."""
    ...
    def __init__(
        ...
        *,
        ...
        crossmix_hidden_ratio: float = 1.0,
        crossmix_hidden_dim: ty.Optional[int] = None,
        crossmix_channel_ratio: float = 2.0,
        crossmix_dropout: ty.Optional[float] = None,
        crossmix_activation: str = "gelu",
        crossmix_layerscale_init: float = 1e-2,
        crossmix_use_channel_mlp: bool = True,
        d_out: int,
    ) -> None:
        super().__init__()
        ...
        cross_hidden = (
            crossmix_hidden_dim
            if crossmix_hidden_dim is not None
            else int(d_token * crossmix_hidden_ratio)
        )
        cross_hidden = max(16, cross_hidden)
        channel_hidden = max(16, int(d_token * crossmix_channel_ratio))
        dropout = (
            crossmix_dropout
            if crossmix_dropout is not None
        ...
        self.layers = nn.ModuleList(
            [
                CrossMixBlock(
                    d_token=d_token,
                    cross_hidden=cross_hidden,
                    channel_hidden=channel_hidden,
                    dropout=dropout or 0.0,
                    layerscale_init=crossmix_layerscale_init,
                    activation=crossmix_activation,
                    use_channel_mlp=crossmix_use_channel_mlp,
                )
                for _ in range(n_layers)
            ]
        )
        self.activation = _resolve_activation(crossmix_activation)
...
class GGPLTMLPCrossMix(GGPLTMLP):
    def __init__(
        self,
        model_config: dict,
        n_num_features: int,
        ...
        if feat_gate or pruning:
            raise NotImplementedError(
                "ggpl_tmlp_crossmix keeps a clean tokenizer replacement and does not support sparse gating options"
            )
        TabModel.__init__(self)
        model_config = self.preproc_config(model_config)
        self.model = _GGPLTMLPCrossMix(
            d_numerical=n_num_features,
            categories=categories,
            d_out=n_labels,
            **model_config,
        ).to(device)
        self.base_name = "ggpl_tmlp_crossmix"
        ...
        self.breakpoint_cache_dir = self.saved_model_config.get(
            "breakpoint_cache_dir", "artifacts/ggpl_tmlp_crossmix_breakpoints"
        )
    ...
    def preproc_config(self, model_config: dict):
        ...
        model_config.setdefault("crossmix_hidden_ratio", 1.0)
        model_config.setdefault("crossmix_hidden_dim", None)
        model_config.setdefault("crossmix_channel_ratio", 2.0)
        model_config.setdefault("crossmix_dropout", None)
        model_config.setdefault("crossmix_activation", "gelu")
        model_config.setdefault("crossmix_layerscale_init", 1e-2)
        model_config.setdefault("crossmix_use_channel_mlp", True)
        return model_config
